...
def aStarSearch(problem):
    ...
    openStates.push((start_state, [], 0), 0)
    ...
    while not openStates.isEmpty():
        ...
        if current_state not in exploredStates:
            exploredStates.add(current_state)
...
def searchWithStopovers(problem, stopovers):
    ...
    hub = current_state
    unvisited_stopovers = stopovers.copy()

    complete_actions = []
    total_cost = 0
    visit_order = []

    # We will visit the stopovers based on their heuristic values (Greedy Best First)

    while unvisited_stopovers:
        closest_stopover = None
        min_heuristic_distance = float('inf')

        for stopover in unvisited_stopovers:
            h_cost = problem.heuristicDict[(current_state, stopover)]

            if h_cost < min_heuristic_distance:
                min_heuristic_distance = h_cost
                closest_stopover = stopover

        # The closest stopover beceome our next destination
        next_goal = closest_stopover
        unvisited_stopovers.remove(next_goal)
        visit_order.append(next_goal)

        # Run A*
        problem.start_state = current_state
        problem.goal_state = next_goal
        path, nodes, cost = aStarSearch(problem)

        # Store results
        complete_actions += path
        total_cost += cost

        # Go to the next destination for the next iteration
        current_state = next_goal

    # Back to the hub
    problem.start_state = current_state
    problem.goal_state = hub
    final_path, final_nodes, final_cost = aStarSearch(problem)

    complete_actions += final_path
    total_cost += final_cost

    return (complete_actions, round(total_cost, 5), visit_order)
